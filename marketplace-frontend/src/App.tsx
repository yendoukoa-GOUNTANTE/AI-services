...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (executionParams.execute) {
      ...
      setExecutionParams(prev => ({ ...prev, execute: false }));
    }
    ...
    try {
      ...
      switch (selectedService?.id) {
        ...
        case 'language-specialist':
          ...
          break;
        case 'elevenlabs':
          response = await aiService.getElevenLabsAssistance(servicePrompt, executionParams.execute, executionParams.voice_id);
          break;
        case 'tiktok-market':
          response = await aiService.getTikTokMarketingAssistance(servicePrompt, executionParams.execute, executionParams.video_url);
          break;
        case 'whatsapp-biz':
          response = await aiService.getWhatsAppBusinessAssistance(servicePrompt, executionParams.execute, executionParams.to_number, executionParams.template_name);
          break;
        case 'cloudinary-media':
          response = await aiService.getCloudinaryMediaAssistance(servicePrompt, executionParams.execute, executionParams.file_id);
          break;
        case 'runway-video':
          response = await aiService.getRunwayVideoAssistance(servicePrompt, executionParams.execute);
          break;
        default:
